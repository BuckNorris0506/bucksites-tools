/**
 * Humidifier reference adapter v1 — read-only projection into UCF contracts.
...
export const COVERAGE_FACTORY_SCHEMA_VERSION_HUMIDIFIER_ADAPTER_V1 = "1.0.0" as const;

export const HUMIDIFIER_COVERAGE_FACTORY_ADAPTER_ID_V1 =
  "humidifier_coverage_factory_reference_adapter_v1" as const;

export const HUMIDIFIER_DATA_DIR_REL_V1 = "data/humidifier" as const;

export const HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1 = {
...
export const HUMIDIFIER_COVERAGE_DISPOSITIONS_V1 = [
...
export type HumidifierCoverageDispositionV1 = (typeof HUMIDIFIER_COVERAGE_DISPOSITIONS_V1)[number];

export type HumidifierCoverageDispositionMappingV1 = {
  humidifier_disposition: HumidifierCoverageDispositionV1;
...
export const HUMIDIFIER_COVERAGE_DISPOSITION_MAPPING_TABLE_V1: readonly HumidifierCoverageDispositionMappingV1[] =
  [
    {
      humidifier_disposition: "NEEDS_RESEARCH_FIRST_NO_BATCH_LANES",
    ...
    {
      humidifier_disposition: "SAMPLE_FILTER_DEMO_INVENTORY",
      core_disposition: "owner_review",
    ...
    {
      humidifier_disposition: "SAMPLE_MODEL_DEMO_INVENTORY",
    ...
    {
      humidifier_disposition: "DEMO_RETAILER_LINK_UNVERIFIED",
    ...
    {
      humidifier_disposition: "SAMPLE_CATALOG_NOT_COMMITTED",
...
export const HUMIDIFIER_COVERAGE_LEGACY_MAP_V1: CoverageLegacyMapV1 = {
  contract: "coverage_legacy_map_v1",
  schema_version: COVERAGE_FACTORY_SCHEMA_VERSION_HUMIDIFIER_ADAPTER_V1,
  entries: HUMIDIFIER_COVERAGE_DISPOSITION_MAPPING_TABLE_V1.map((row) => ({
    legacy_label: row.humidifier_disposition,
...
export type HumidifierLoadedSubjectArtifactsV1 = {
  subject_slug: string;
...
export type HumidifierCoverageFactoryProjectionV1 = {
  adapter_id: typeof HUMIDIFIER_COVERAGE_FACTORY_ADAPTER_ID_V1;
  schema_version: typeof COVERAGE_FACTORY_SCHEMA_VERSION_HUMIDIFIER_ADAPTER_V1;
  read_only: true;
  data_mutation: false;
  ...
  disposition_mappings: readonly HumidifierCoverageDispositionMappingV1[];
  source_artifact_paths: string[];
};

export type HumidifierProjectionReportRowV1 = {
  subject_slug: string;
  subject_kind: "filter" | "model";
...
export type HumidifierContractFitGapKindV1 =
  | "PROVEN_CONTRACT_GAP"
  | "ADAPTER_ONLY"
  | "LEGACY_DATA_ISSUE";

export type HumidifierContractFitGapV1 = {
  kind: HumidifierContractFitGapKindV1;
  topic: string;
  detail: string;
};

export function mapHumidifierDispositionToUcfV1(
  humidifierDisposition: HumidifierCoverageDispositionV1,
): HumidifierCoverageDispositionMappingV1 {
  const row = HUMIDIFIER_COVERAGE_DISPOSITION_MAPPING_TABLE_V1.find(
    (entry) => entry.humidifier_disposition === humidifierDisposition,
  );
  if (!row) {
    throw new Error(`Unknown humidifier disposition: ${humidifierDisposition}`);
  }
  return row;
}
...
function postureContractRowRef(): CoverageProvenanceRefV1 {
  return {
    kind: "contract_row",
    contract: "humidifier_wedge_posture_snapshot_v1",
    row_key: HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1.feasibility_recommendation,
  };
}
...
export function resolveHumidifierDispositionV1(
  loaded: HumidifierLoadedSubjectArtifactsV1,
): HumidifierCoverageDispositionV1 {
  ...
  if (
    ...
    HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1.feasibility_recommendation === "NEEDS_RESEARCH_FIRST"
  ) {
    return "DEMO_RETAILER_LINK_UNVERIFIED";
  }

  if (!HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1.batch_production_lanes_committed) {
...
function blockersFromLoaded(loaded: HumidifierLoadedSubjectArtifactsV1): string[] {
  const blockers = [
    "sample_csv_only:not_committed_catalog",
    `wedge_posture:${HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1.truth_coverage_status}`,
    `public_indexing:${HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1.public_indexing_status}`,
    `feasibility:${HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1.feasibility_recommendation}`,
  ];
...
function deriveEvidenceStatuses(
  loaded: HumidifierLoadedSubjectArtifactsV1,
  mapping: HumidifierCoverageDispositionMappingV1,
): Record<
...
function buildSubjectForHumidifier(loaded: HumidifierLoadedSubjectArtifactsV1): CoverageSubjectV1 {
  ...
  if (!loaded.filter_row) {
    throw new Error(`Missing humidifier filter row for ${loaded.subject_slug}`);
  }
...
function buildEvidenceForHumidifierSubject(
  ...
  return {
    ...
    claims: {
      ...
      publication: claim("publication", HUMIDIFIER_WEDGE_POSTURE_SNAPSHOT_V1.public_indexing_status),
    },
    read_only: true,
    data_mutation: false,
  };
...
function buildAssessmentForHumidifierSubject(
  subject: CoverageSubjectV1,
  loaded: HumidifierLoadedSubjectArtifactsV1,
  mapping: HumidifierCoverageDispositionMappingV1,
...
function buildWorkItemForHumidifierSubject(
  subject: CoverageSubjectV1,
  loaded: HumidifierLoadedSubjectArtifactsV1,
  mapping: HumidifierCoverageDispositionMappingV1,
): CoverageWorkItemV1 {
  return {
    ...
    work_item_id: `humidifier-ucf-${subject.internal_slug_labels[0]}`,
...
function buildSubjectLinksForHumidifier(
  bundle: SampleCsvWedgeBundleV1,
...
export function loadHumidifierArtifactsForSubjectSlugV1(
  rootDir: string,
  subjectSlug: string,
): HumidifierLoadedSubjectArtifactsV1 {
  ...
  if (!filter_row && !model_row) {
    throw new Error(`No committed humidifier sample CSV artifacts found for ${subjectSlug}`);
  }
  ...
  const loaded: HumidifierLoadedSubjectArtifactsV1 = {
...
export function projectHumidifierLoadedArtifactsV1(
  rootDir: string,
  loaded: HumidifierLoadedSubjectArtifactsV1,
): Omit<
  HumidifierCoverageFactoryProjectionV1,
  "run_manifest" | "disposition_mappings" | "source_artifact_paths"
> {
  const humidifierDisposition = resolveHumidifierDispositionV1(loaded);
  const mapping = mapHumidifierDispositionToUcfV1(humidifierDisposition);
  const subject = buildSubjectForHumidifier(loaded);
  const provenance_refs = provenanceRefsFromBundle(rootDir, loaded.bundle);

  return {
    adapter_id: HUMIDIFIER_COVERAGE_FACTORY_ADAPTER_ID_V1,
    schema_version: COVERAGE_FACTORY_SCHEMA_VERSION_HUMIDIFIER_ADAPTER_V1,
    read_only: true,
    data_mutation: false,
    subjects: [subject],
    evidence: [buildEvidenceForHumidifierSubject(subject, loaded, mapping, provenance_refs)],
    assessments: [buildAssessmentForHumidifierSubject(subject, loaded, mapping)],
    work_items: [buildWorkItemForHumidifierSubject(subject, loaded, mapping)],
    subject_links: buildSubjectLinksForHumidifier(loaded.bundle).filter(
      (link) =>
...
export function buildHumidifierCoverageFactoryReferenceProjectionV1(args: {
  rootDir: string;
  subjectSlugs: string[];
  now?: () => Date;
}): HumidifierCoverageFactoryProjectionV1 {
  ...
  subject_links.push(...buildSubjectLinksForHumidifier(bundle));

  for (const subjectSlug of args.subjectSlugs) {
    const loaded = loadHumidifierArtifactsForSubjectSlugV1(args.rootDir, subjectSlug);
    const partial = projectHumidifierLoadedArtifactsV1(args.rootDir, loaded);
  ...
  const run_manifest: CoverageRunManifestV1 = {
    ...
    schema_version: COVERAGE_FACTORY_SCHEMA_VERSION_HUMIDIFIER_ADAPTER_V1,
    run_id: `humidifier-ucf-reference-${now().toISOString().slice(0, 10)}`,
    adapter_id: HUMIDIFIER_COVERAGE_FACTORY_ADAPTER_ID_V1,
  ...
  return {
    ...
    subject_links,
    run_manifest,
    disposition_mappings: HUMIDIFIER_COVERAGE_DISPOSITION_MAPPING_TABLE_V1,
    source_artifact_paths,
  };
}

export function buildHumidifierProjectionReportV1(
  projection: HumidifierCoverageFactoryProjectionV1,
  rootDir?: string,
): HumidifierProjectionReportRowV1[] {
  return projection.subjects.map((subject, index) => {
    ...
    const loaded = rootDir
      ? loadHumidifierArtifactsForSubjectSlugV1(rootDir, slug)
      : null;
    const humidifierDisposition = loaded
      ? resolveHumidifierDispositionV1(loaded)
      : (assessment.adapter_state as HumidifierCoverageDispositionV1);
...
export function assessHumidifierContractFitV1(): HumidifierContractFitGapV1[] {
  return [
    {
      kind: "ADAPTER_ONLY",
      topic: "sample_csv_only_inventory",
      detail:
        "Humidifier has no committed batch-production JSON lanes; adapter projects sample CSV rows plus inlined wedge posture snapshot.",
    },
    {
      kind: "ADAPTER_ONLY",
      topic: "wicking_filter_replacement_interval",
      detail:
        "Humidifier replacement_interval_months from sample CSV cannot be stored on CoverageSubjectV1; adapter surfaces interval only via evidence summary when present.",
    },
    {
      kind: "ADAPTER_ONLY",
    ...
    },
    {
      ...
      detail:
        "Only humi-wf50 / humi-h100 sample rows exist; pressure test cannot sample multi-slug batch-factory behavior.",
    },
  ];
}

export function humidifierCoverageDispositionMeaningPreservedV1(args: {
  humidifierDisposition: HumidifierCoverageDispositionV1;
  assessment: CoverageAssessmentV1;
}): boolean {
  const mapping = mapHumidifierDispositionToUcfV1(args.humidifierDisposition);
  return (
