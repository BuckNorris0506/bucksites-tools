...
describe("homepage featured fit provenance", () => {
  it("binds the featured case to CSV mapping, model, part, and manufacturer evidence", () => {
    ...
    assert.ok(evidence.notes.includes("HAF-CIN"));
    assert.ok(evidence.notes.includes("DA29-00020B"));
    assert.ok(FEATURED_FIT_CONDITION.includes(evidence.cautions));
    assert.ok(
      audit.includes(
    ...
    assert.equal(FEATURED_FIT_VERDICT, `Fits ${FEATURED_FIT_MODEL_NUMBER}.`);
    assert.ok(FEATURED_FIT_REASON.includes("HAF-CIN"));
    assert.equal(FEATURED_FIT_HREF, `/fridge/${FEATURED_FIT_FRIDGE_SLUG}`);
  });

  it("does not put price, retailer CTA, or invented metrics in the featured example module", () => {
    const ui = readFileSync(root("src", "components", "homepage", "FeaturedFitExample.tsx"), "utf8");
...
describe("homepage exact copy", () => {
  it("locks the approved hero strings", () => {
    assert.equal(HOME_EYEBROW, "Replacement-part fit checks");
    assert.equal(HOME_H1, "Know what fits before you buy.");
    assert.equal(
      HOME_SUBHEADLINE,
      "BuckParts checks replacement-part fit and shows you the evidence. We’re not a parts store.",
    );
    assert.equal(HOME_SCOPE, "Starting with refrigerator water and air purifier filters.");
    assert.equal(HOME_INPUT_LABEL, "Appliance model or part number");
    assert.equal(HOME_PRIMARY_CTA, "Find my part");
    assert.equal(HOME_SECOND_DOOR, "Help me find the number");
    assert.equal(HOME_TRUST_LINE, "Fit first. Buying options second.");
  });

  it("homepage markup uses the 7/5 hero composition and omits banned shelves", () => {
    const page = readFileSync(root("src", "app", "page.tsx"), "utf8");
    const css = readFileSync(root("src", "app", "homepage.css"), "utf8");
    assert.ok(css.includes("grid-column: span 7"));
    assert.ok(css.includes("grid-column: span 5"));
    assert.ok(css.includes("gap: 24px"));
