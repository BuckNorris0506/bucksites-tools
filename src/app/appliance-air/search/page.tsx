...
import {
  enrichApplianceAirModelHitsWithFilters,
  searchApplianceAirCatalog,
} from "@/lib/data/appliance-air/search";
...
export default async function ApplianceAirSearchPage({ searchParams }: Props) {
  const query = searchParams.q?.trim() ?? "";
  let error: string | null = null;
  let hits: Awaited<ReturnType<typeof searchApplianceAirCatalog>> = [];

  if (query.length >= 2) {
    try {
      const raw = await searchApplianceAirCatalog(query);
      hits = await enrichApplianceAirModelHitsWithFilters(raw);
    } catch (e) {
      error =
    ...
    <div className="space-y-10">
      <div className="space-y-4">
        <p className="text-sm text-neutral-500">
          <Link href="/appliance-air">← Appliance air home</Link>
        </p>
        <h1 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-3xl">
          Search appliance air filters
        </h1>
        <SearchForm initialQuery={query} actionPath="/appliance-air/search" />
      </div>
