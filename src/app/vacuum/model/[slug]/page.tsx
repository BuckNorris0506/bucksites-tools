...
import { getVacuumModelBySlug } from "@/lib/data/vacuum/models";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const model = await getVacuumModelBySlug(params.slug);
  if (!model) {
    return { title: "Model not found" };
  }
  return {
    title: `${model.model_number} vacuum filters`,
    description: `Replacement filters for ${model.brand.name} model ${model.model_number}.`,
  };
}

export default async function VacuumModelPage({ params }: Props) {
  const model = await getVacuumModelBySlug(params.slug);
  if (!model) notFound();
