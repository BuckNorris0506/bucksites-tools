...
import {
  enrichHumidifierModelHitsWithFilters,
  searchHumidifierCatalog,
} from "@/lib/data/humidifier/search";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (!query) {
    return { title: "Search humidifier filters" };
  }
  return {
    title: `Humidifier search: ${query}`,
    description: `Results for humidifier models and filters matching “${query}”.`,
...
export default async function HumidifierSearchPage({ searchParams }: Props) {
  const query = searchParams.q?.trim() ?? "";
  let error: string | null = null;
  let hits: Awaited<ReturnType<typeof searchHumidifierCatalog>> = [];
    ...
    <div className="space-y-10">
      <div className="space-y-4">
        <p className="text-sm text-neutral-500 dark:text-neutral-400">
          <Link href="/humidifier" className="hover:text-neutral-800 dark:hover:text-neutral-200">
            ← Humidifier home
          </Link>
        </p>
        <h1 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-50 sm:text-3xl">
          Search humidifier filters
        </h1>
        <SearchForm initialQuery={query} actionPath="/humidifier/search" />
      </div>
            ...
            <section className="space-y-3">
              ...
              <ul className="space-y-2">
                {models.map((hit) => (
                  <li key={hit.slug}>
                    <Link
                      href={`/humidifier/model/${hit.slug}`}
            ...
            <section className="space-y-3">
              ...
              <ul className="space-y-2">
                {filters.map((hit) => (
                  <li key={hit.slug}>
                    <Link
                      href={`/humidifier/filter/${hit.slug}`}
                      className="block rounded-lg border border-neutral-200 bg-white p-4 transition-colors hover:border-neutral-300 hover:bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-950 dark:hover:border-neutral-700 dark:hover:bg-neutral-900/80"
                    >
