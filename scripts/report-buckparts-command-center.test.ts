...
test("next_best_action does not claim no non-Amazon APPROVED when Waterdrop LIVE evidence exists", async () => {
  ...
  if (issueReauditSteeringActive(report)) {
    assert.match(report.next_best_action, /BP-000002/);
...
test("command_center_v2.command_center_issue_reaudit_v1 surfaces deployed re-audit candidates", async () => {
  ...
  assert.equal(reaudit.top_reaudit_candidate?.issue_id, "BP-000002");
  assert.ok(reaudit.top_reaudit_candidate?.suggested_hyperagent_prompt.includes("BP-000002"));
  ...
  assert.equal(registry.total_closed, 1);
  assert.deepEqual(registry.closed_proven_issue_ids, ["BP-000001"]);
  assert.match(report.next_best_action, /ISSUE RE-AUDIT: BP-000002/);
  assert.match(report.why_this_action, /re-audit|RE_AUDIT/i);
  assert.equal(
