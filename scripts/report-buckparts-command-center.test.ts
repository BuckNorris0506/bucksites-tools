...
test("command center next_best_action prefers demand-selected batch when refrigerator_water lifecycle is closed", async () => {
  ...
  if (
    ...
    assert.equal(demandSelectedCorrectnessRisksSteeringActive(report), false);
  }
...
test("command_center_v2.command_center_issue_reaudit_v1 surfaces deployed re-audit candidates", async () => {
  ...
  assert.equal(reaudit.total_deployed_awaiting_reaudit, 0);
  assert.equal(reaudit.top_reaudit_candidate, null);
  assert.equal(reaudit.candidates.length, 0);
  ...
  assert.equal(registry.total_closed, 6);
...
test("command_center_v2.air_purifier_demand_selected_correctness_risks_v1 is read-only audit projection", async () => {
  ...
  if (lane.source_status === "PROVEN") {
    ...
    assert.equal(lane.generated_at, "2026-06-24T02:00:00.000Z");
    assert.match(lane.recommended_action, /demand_to_coverage|demand-selected correctness risks cleared/i);
  } else {
...
test("command_center_v2.wrong_code_prevention_v1 is read-only HyperAgent findings lane", async () => {
  const report = await buildBuckpartsCommandCenterReport({
    providers: baseProviders(),
    liveSiteMonitor: null,
    demandToCoverageEngineLoader: async () => buildDemandToCoverageEngineV1FromRows([], "OK", []),
  ...
  const lane = report.command_center_v2.wrong_code_prevention_v1;
  assert.ok(lane);
  assert.equal(lane.contract, "wrong_code_prevention_v1");
  assert.equal(lane.read_only, true);
  assert.equal(lane.data_mutation, false);
  ...
  assert.equal(lane.recommended_jq_path, ".command_center_v2.wrong_code_prevention_v1");
  assert.equal(lane.artifact_path, "data/command-center/audits/wrong-code-prevention-v1.json");
  assert.ok(["PASS", "WARN", "FAIL", "UNKNOWN"].includes(lane.overall_status));
});
...
test("command_center_v2.manufacturer_safe_link_rescue_runner_v1 is read-only rescue execution lane", async () => {
  const report = await buildBuckpartsCommandCenterReport({
    providers: baseProviders(),
    fileExists: fs.existsSync,
    readDir: (p) => fs.readdirSync(p),
  ...
  const lane = report.command_center_v2.manufacturer_safe_link_rescue_runner_v1;
  assert.ok(lane);
  assert.equal(lane.contract, "manufacturer_safe_link_rescue_runner_v1");
  assert.equal(lane.read_only, true);
  assert.equal(lane.data_mutation, false);
  assert.equal(lane.coverage_unlocked, false);
  assert.equal(lane.csv_apply_authorized, false);
  ...
  if (lane.slug_states.length > 0) {
    assert.equal(lane.slug_states.filter((s) => s.stage === "READY_FOR_APPLY").length <= 1, true);
    assert.ok(lane.execution_order.length > 0);
    assert.ok(lane.manufacturer_workloads.length > 0);
  }
  assert.ok(
    lane.inspect_summary.recommended_jq_paths.command_center.includes(
