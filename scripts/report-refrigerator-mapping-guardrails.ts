...
async function main() {
  ...
  const [
    ...
    >(supabase, "fridge_models", "id, slug, model_number, brands:brand_id(slug, name)", pageSize),
    selectPaged<{ fridge_model_id: string; filter_id: string }>(
      supabase,
      "compatibility_mappings",
      "fridge_model_id, filter_id",
      pageSize,
    ),
    selectPaged<{ id: string; slug: string; oem_part_number: string; brands: { slug: string; name: string } | null }>(
      supabase,
      "filters",
      "id, slug, oem_part_number, brands:brand_id(slug, name)",
      pageSize,
    ),
    selectPaged<{ id: string; filter_id: string; is_primary: boolean | null }>(
      supabase,
      "retailer_links",
      "id, filter_id, is_primary",
      pageSize,
    ),
    supabase
  ...
  const modelsWithCompat = new Set(compatRows.map((r) => r.fridge_model_id));
  const filterIdsWithAnyLink = new Set(retailerLinks.map((r) => r.filter_id));

  const primaryByFilter = new Map<string, string[]>();
  for (const row of retailerLinks) {
    if (row.is_primary !== true) continue;
    const list = primaryByFilter.get(row.filter_id) ?? [];
    list.push(row.id);
    primaryByFilter.set(row.filter_id, list);
  }
  const duplicatePrimary = Array.from(primaryByFilter.entries())
    .filter(([, ids]) => ids.length > 1)
    .map(([filter_id, link_ids]) => {
      const f = filters.find((x) => x.id === filter_id);
      return {
        filter_id,
  ...
  const filtersReferencedByCompat = new Set(compatRows.map((r) => r.filter_id));
  const compatFiltersMissingRetailerLink = Array.from(filtersReferencedByCompat).filter(
    (fid) => !filterIdsWithAnyLink.has(fid),
  );
  const compatFiltersMissingLinkDetail = compatFiltersMissingRetailerLink.map((filter_id) => {
    const f = filters.find((x) => x.id === filter_id);
    return {
      filter_id,
      filter_slug: f?.slug ?? null,
  ...
  const liveModelsWithoutCompat = fridgeModels
    .filter((m) => !modelsWithCompat.has(m.id))
    .map((m) => ({
      fridge_model_id: m.id,
      slug: m.slug,
  ...
  for (const s of stagedModels) {
    ...
    if (!modelsWithCompat.has(row.id)) {
      stagedModelsMatchingLiveWithoutCompat.push({
        ...
        proposed_model_number: s.proposed_model_number,
        proposed_brand_slug: s.proposed_brand_slug,
        fridge_model_id: row.id,
        fridge_slug: row.slug,
      });
    }
  }
...
main().catch((err) => {
  console.error("[report-refrigerator-mapping-guardrails] failed", err);
  process.exit(1);
});
