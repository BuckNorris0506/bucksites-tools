/**
 * CSV → Supabase for humidifier vertical only.
...
async function main() {
  log("import-humidifier", `Starting (dry_run=${!write}, sample=${useSample})`);
  try {
    const result = await runVerticalSeedV1({
      rootDir: cwd,
      verticalKey: HOMEKEEP_WEDGE_CATALOG.humidifier,
      useSample,
      write,
    ...
    if (result.report.apply_status === "BLOCKED") {
      console.error("[import-humidifier] BLOCKED");
    } else if (!write) {
      log("import-humidifier", "Dry-run complete.");
    } else {
