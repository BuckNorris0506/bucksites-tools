...
test("deployed issue without re_audit_outcome becomes re-audit candidate", () => {
  ...
  assert.equal(lane.total_deployed_awaiting_reaudit, 3);
  assert.equal(lane.candidates.length, 3);
...
test("TIER_0 oldest deployed issue selected as top_reaudit_candidate", () => {
  ...
  assert.equal(lane.top_reaudit_candidate?.issue_id, "BP-000002");
  ...
  assert.equal(top?.issue_id, "BP-000002");

  const tier0Open = issues.filter(
    (i) => i.severity === "TIER_0" && i.status !== "CLOSED_PROVEN",
  );
  const sorted = sortCommandCenterIssuesByPriorityV1(tier0Open);
  assert.equal(sorted[0]?.issue_id, "BP-000002");
});
...
test("DEPLOYED issues do not steer repair NBA but re-audit steering activates", () => {
  ...
  assert.equal(reaudit.total_deployed_awaiting_reaudit, 3);

  const steering = resolveCommandCenterIssueReauditSteeringOverrideV1(reaudit);
  assert.ok(steering);
  assert.match(steering!.next_best_action, /ISSUE RE-AUDIT: BP-000002/);
