...
test("deployed issue without re_audit_outcome becomes re-audit candidate", () => {
  ...
  assert.equal(lane.total_deployed_awaiting_reaudit, 4);
  assert.equal(lane.candidates.length, 4);
...
test("TIER_0 oldest deployed issue selected as top_reaudit_candidate", () => {
  const lane = buildCommandCenterIssueReauditLaneV1({
    rootDir: ROOT,
    issue_registry: registryLane(),
  });
  assert.equal(lane.top_reaudit_candidate?.issue_id, "BP-000001");
  assert.equal(lane.top_reaudit_candidate?.severity, "TIER_0");
  ...
  assert.equal(top?.issue_id, "BP-000001");

  const tier0 = issues.filter((i) => i.severity === "TIER_0");
  const sorted = sortCommandCenterIssuesByPriorityV1(tier0);
  assert.equal(sorted[0]?.issue_id, "BP-000001");
});
...
test("DEPLOYED issues do not steer repair NBA but re-audit steering activates", () => {
  ...
  assert.ok(steering);
  assert.match(steering!.next_best_action, /ISSUE RE-AUDIT: BP-000001/);
