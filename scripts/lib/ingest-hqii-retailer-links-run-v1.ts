/**
 * HQII retailer-links ingest — run orchestration with truth-ledger outcome recording.
 */
...
import {
  ...
  INGEST_HQII_RETAILER_LINKS_MUTATION_LANE_V1,
  ingestHqiiRetailerLinksMutationAuthorizedV1,
  type IngestHqiiRetailerLinksMutationPreflightV1,
...
export type IngestHqiiRetailerLinksApplyStatusV1 = "BLOCKED" | "APPLIED";

export type IngestHqiiRetailerLinksReportV1 = {
  wedge: string;
  dry_run: boolean;
  input_count: number;
  ...
  post_insert_failures?: Array<{ filter_slug: string; reason: string }>;
  final_cta_winner?: Array<{
    filter_slug: string;
    filter_id: string;
    winner: { id: string; retailer_key: string | null; affiliate_url: string } | null;
    surviving_link_count: number;
  }>;
  apply_status?: IngestHqiiRetailerLinksApplyStatusV1;
  mutation_authorized?: boolean;
  mutation_preflight_blockers?: string[];
  founder_decision_id?: string | null;
};

export type IngestHqiiRetailerLinksRunResultV1 = {
  report: IngestHqiiRetailerLinksReportV1;
  exit_code: 0 | 1;
  unknown_retailer_warnings: string[];
};

export type IngestHqiiRetailerLinksDepsV1 = {
...
function parseInputJson(rootDir: string, jsonPath: string, readText?: (abs: string) => string): HqIIRawDiscovery[] {
  const abs = path.resolve(rootDir, jsonPath);
  const body = readText ? readText(abs) : fs.readFileSync(abs, "utf8");
...
export function parseIngestHqiiRetailerLinksCliArgsV1(argv: readonly string[]): {
  inputPath: string | null;
  write: boolean;
  wedge: string | null;
  allowUnknownRetailers: boolean;
} {
  ...
  return {
    ...
    wedge: argValue("--wedge"),
    allowUnknownRetailers: argv.includes("--allow-unknown-retailer"),
  };
}

export function createIngestHqiiRetailerLinksLiveDepsV1(
  getSupabaseAdmin: () => SupabaseClient,
): IngestHqiiRetailerLinksDepsV1 {
  return { getSupabaseAdmin };
}

async function computePostInsertWinnersV1(args: {
  deps: IngestHqiiRetailerLinksDepsV1;
  cfg: TableConfig;
  filterBySlug: Map<string, string>;
  filterIds: string[];
}): Promise<{
  winners: NonNullable<IngestHqiiRetailerLinksReportV1["final_cta_winner"]>;
  failedPostInsert: NonNullable<IngestHqiiRetailerLinksReportV1["post_insert_failures"]>;
}> {
  const supabase = args.deps.getSupabaseAdmin();
  const { data: postRows, error: postErr } = await supabase
    .from(args.cfg.retailerLinksTable)
    .select(
      `${args.cfg.retailerFilterFk}, id, retailer_name, retailer_key, affiliate_url, browser_truth_classification, browser_truth_checked_at`,
    )
    .in(args.cfg.retailerFilterFk, args.filterIds);
  if (postErr) throw postErr;

  const byFilter = new Map<string, Array<Record<string, unknown>>>();
  for (const row of (postRows ?? []) as Array<Record<string, unknown>>) {
    const filterId = String(row[args.cfg.retailerFilterFk] ?? "");
    if (!byFilter.has(filterId)) byFilter.set(filterId, []);
    byFilter.get(filterId)!.push(row);
  }

  const winners: NonNullable<IngestHqiiRetailerLinksReportV1["final_cta_winner"]> = [];

  for (const [slug, filterId] of args.filterBySlug.entries()) {
    if (!args.filterIds.includes(filterId)) continue;
  ...
  const failedPostInsert = winners
    .filter((w) => w.surviving_link_count < 1)
    .map((w) => ({
      filter_slug: w.filter_slug,
      reason: "no_live_buyable_link_after_gate",
    }));

  return { winners, failedPostInsert };
}

export async function runIngestHqiiRetailerLinksV1(args: {
  rootDir: string;
  inputPath: string;
  write: boolean;
  wedgeArg: string;
...
}): Promise<IngestHqiiRetailerLinksRunResultV1> {
  const now = args.now ?? (() => new Date());
  const recordTruthLedger = args.recordTruthLedger ?? recordTruthLedgerMutationOutcomeV1;
  const warn = args.deps.warn ?? (() => {});

  if (!isHomekeepWedgeCatalog(args.wedgeArg)) {
    throw new Error(`Invalid --wedge "${args.wedgeArg}"`);
  }
  const cfg = TABLES[args.wedgeArg];
  if (!cfg) throw new Error(`Unsupported wedge "${args.wedgeArg}"`);

  const raw = parseInputJson(args.rootDir, args.inputPath, args.readText);
  ...
  for (const row of raw) {
    ...
    if (!canonicalRetailer && !args.allowUnknownRetailers) {
  ...
  const baseReport: IngestHqiiRetailerLinksReportV1 = {
    wedge: cfg.wedge,
    dry_run: !args.write,
  ...
  if (!args.write) {
    return {
      report: baseReport,
      exit_code: 0,
      unknown_retailer_warnings: unknownWarnings,
    };
  }

  const preflight: IngestHqiiRetailerLinksMutationPreflightV1 =
    buildIngestHqiiRetailerLinksMutationPreflightV1({
  ...
  const mutation_authorized = ingestHqiiRetailerLinksMutationAuthorizedV1(preflight);
  const blockers = [...preflight.blockers];

  let winners: IngestHqiiRetailerLinksReportV1["final_cta_winner"];
  let failedPostInsert: IngestHqiiRetailerLinksReportV1["post_insert_failures"];
  ...
  if (mutation_authorized) {
    ...
    if (filterIds.length > 0) {
      const post = await computePostInsertWinnersV1({
        deps: args.deps,
        cfg,
        filterBySlug,
        filterIds,
      ...
      failedPostInsert = post.failedPostInsert;
    }
  }

  let apply_status: IngestHqiiRetailerLinksApplyStatusV1 =
  ...
  for (const w of unknownWarnings) warn(w);
