...
function proposedMutationForRow(args: {
  ...
  if (repairGroup === "wf2cb_partial_needs_browser_proof") {
...
function groupRowsByRepairGroup(
  ...
  return REPAIR_GROUPS_V1.map((repair_group) => {
    ...
    const batch_id =
      slug_rows[0]?.source_batch_id ??
      (repair_group.startsWith("samsung")
        ? "samsung-bad-mapping-batch-001"
        : "wf2cb-bounded-evidence-slice-001");
    return {
      repair_group,
      batch_id,
...
export function buildRefrigeratorTruthRepairOwnerReviewV1(args: {
  ...
  return {
    ...
    proven_facts: [
      `PROVEN: Loaded ${String(slugRows.length)} slug rows from Samsung bad-mapping batch 001 and WF2CB bounded slice 001 cursor validation packets.`,
      `PROVEN: apply_candidate_count=${String(summary.apply_candidate_count)} (samsung_pass_ready only).`,
      `PROVEN: browser_proof_required_count=${String(summary.browser_proof_required_count)} (PARTIAL verdict rows).`,
