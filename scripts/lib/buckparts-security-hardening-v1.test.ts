...
describe("security hardening v1", () => {
  ...
  test("search_gaps P2 writers pass service-role inventory guarded audit", () => {
  ...
  test("slice 6 writers pass service-role inventory guarded audit with zero write_unguarded", () => {
    ...
    assert.equal(guarded.length, 28);
    assert.ok(
      guarded.some(
        (e) =>
          e.rel_path ===
    ...
    assert.ok(
      guarded.some(
        (e) =>
          e.rel_path ===
            "scripts/lib/buckparts-retailer-link-parity-guarded-apply-v1.ts" &&
          e.mutation_lane === "retailer_link_parity_guarded_apply_v1",
      ),
      "retailer-link parity guarded apply writer must be write_guarded in inventory",
    );
