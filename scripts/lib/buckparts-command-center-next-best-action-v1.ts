...
export function resolveCommandCenterNextBestActionV1(
  ...
  if (input.preferAmazonFirstConversion) {
  ...
  } else if (!input.topMoneyQueue[0]?.exhausted && input.topMoneyQueue[0]?.candidate_count !== "UNKNOWN") {
    ...
    why_this_action = "FlexOffers readiness queue remains available as the next monetization-prep lane.";
  } else {
