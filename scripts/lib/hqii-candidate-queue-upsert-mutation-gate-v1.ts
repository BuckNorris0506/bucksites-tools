/**
 * HQII candidate queue upsert mutation gate — founder approval, trust currency, input artifact binding.
 */
...
export type HqiiCandidateQueueUpsertMutationGateModeV1 = Extract<
  SupabaseMutationGateModeV1,
  "dry_run" | "write"
>;

export const HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_GATE_CONTRACT_V1 =
  "hqii_candidate_queue_upsert_mutation_gate_v1" as const;

export const HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_GATE_REF_V1 =
  "hqii_candidate_queue_upsert_mutation_gate_v1" as const;

export const HQII_CANDIDATE_QUEUE_UPSERT_PLAN_REL_V1 =
  "scripts/hqii-candidate-queue-upsert.ts";

export const HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_LANE_V1 =
  "hqii_candidate_queue_upsert_v1" as const;

export const HQII_CANDIDATE_QUEUE_UPSERT_IO_READ_INDEX_SUPABASE_BLOCKER_V1 =
  "io_capability_read_index_cannot_mutate_supabase" as const;

export type HqiiCandidateQueueUpsertMutationPreflightV1 = {
  contract: typeof HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_GATE_CONTRACT_V1;
  read_only: true;
  mode: HqiiCandidateQueueUpsertMutationGateModeV1;
  mutation_authorized: boolean;
  blockers: string[];
  founder_decision_id: string | null;
  io_capability: BuckpartsIoCapabilityV1;
  apply_plan_rel: typeof HQII_CANDIDATE_QUEUE_UPSERT_PLAN_REL_V1;
  input_artifact_rel: string | null;
  mutation_lane: typeof HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_LANE_V1;
  mutationGateRef: typeof HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_GATE_REF_V1;
};
...
export function founderRowAuthorizesHqiiCandidateQueueUpsertPlanV1(args: {
  planRel: string;
...
export function founderRowBindsHqiiInputArtifactV1(args: {
...
export function findActiveFounderDecisionForHqiiCandidateQueueUpsertV1(args: {
  rootDir: string;
  planRel: string;
  inputRelPath: string;
  nowIso: string;
  ...
  for (const loaded of founderRows) {
    ...
    const inputBind = founderRowBindsHqiiInputArtifactV1({
      row,
...
export function buildHqiiCandidateQueueUpsertMutationPreflightV1(args: {
  rootDir: string;
  mode: HqiiCandidateQueueUpsertMutationGateModeV1;
  ...
  readText?: (abs: string) => string;
  founderRows?: readonly FounderDecisionRowWithSlugCorrelationV1[];
}): HqiiCandidateQueueUpsertMutationPreflightV1 {
  const io_capability = args.io_capability ?? resolveIoCapabilityFromEnvV1();
  const now = args.now ?? (() => new Date());
  const planRel = args.planRel ?? HQII_CANDIDATE_QUEUE_UPSERT_PLAN_REL_V1;
  const inputRel =
  ...
  if (args.mode === "dry_run") {
    return {
      contract: HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_GATE_CONTRACT_V1,
      read_only: true,
      mode: args.mode,
      ...
      io_capability,
      apply_plan_rel: HQII_CANDIDATE_QUEUE_UPSERT_PLAN_REL_V1,
      input_artifact_rel: inputRel,
      mutation_lane: HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_LANE_V1,
      mutationGateRef: HQII_CANDIDATE_QUEUE_UPSERT_MUTATION_GATE_REF_V1,
    };
  }

  if (io_capability === "READ_INDEX") {
    blockers.push(HQII_CANDIDATE_QUEUE_UPSERT_IO_READ_INDEX_SUPABASE_BLOCKER_V1);
  }
  ...
  if (inputRel) {
    founderRow = findActiveFounderDecisionForHqiiCandidateQueueUpsertV1({
      rootDir: args.rootDir,
      planRel,
...
export function hqiiCandidateQueueUpsertMutationAuthorizedV1(
  preflight: HqiiCandidateQueueUpsertMutationPreflightV1,
): boolean {
  return preflight.mode === "write" && preflight.mutation_authorized;
}

export function assertHqiiCandidateQueueUpsertWriteAuthorizedV1(
  preflight: HqiiCandidateQueueUpsertMutationPreflightV1,
): void {
