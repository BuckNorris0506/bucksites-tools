/**
 * Read-only operator runbook: refrigerator_water pipeline + data health snapshot.
 */
import { loadEnv } from "./lib/load-env";
import {
  classifyRefrigeratorSearchGap,
  type RefrigeratorGapState,
...
async function main() {
  ...
  const [
    ...
    supabase.from("fridge_models").select("id", { count: "exact", head: true }),
  ]);
  ...
  const errs = [
    ...
    totalFridgeModelsCount.error,
  ].filter(Boolean);
  if (errs.length) throw errs[0];

  const linkFilterIds = await pagedColumnIds("retailer_links", "filter_id");
  const compatFilterIds = await pagedColumnIds("compatibility_mappings", "filter_id");
  const usefulFilterIds = new Set([...compatFilterIds, ...linkFilterIds]);

  const mappedWithoutRetailerIds = [...compatFilterIds].filter((id) => !linkFilterIds.has(id));
  ...
  for (let i = 0; i < mappedWithoutRetailerIds.length && compatNoRetailerSample.length < sampleTop; i += 80) {
  ...
  const gapClassificationTally: Record<RefrigeratorGapState, number> = {
    still_unresolved: 0,
  ...
  const search_gaps_reviewed: Array<
    Record<string, unknown> & {
      gap_classification: { state: RefrigeratorGapState; detail: string };
    }
  > = [];
  for (const row of searchGapsRes.data ?? []) {
    ...
    const gap_classification = await classifyRefrigeratorSearchGap({
  ...
  if (nResolved > 0) {
    recommended_next_actions.push(
      `${nResolved} refrigerator-related search_gap row(s) are stale: /search (prod behavior, discovery gating applied) already returns hits. Plan a DB update to status=resolved — this runbook is read-only.`,
    );
  }
  if (nHiddenOrphan > 0) {
    recommended_next_actions.push(
      `${nHiddenOrphan} gap(s): OEM matches a live filter row with no compatibility_mappings and no retailer_links, so it stays hidden from browse/search. Add maps + buy links or mark gap ignored if intentional.`,
    );
  }
  if (nSuperseded > 0) {
    recommended_next_actions.push(
      `${nSuperseded} gap(s): live fridge_models or mapped filters exist for normalized tokens but /search still empty — re-check norms/RPCs or close gap after manual verification.`,
    );
  }
  if (nStill > 0) {
    recommended_next_actions.push(
      `${nStill} gap(s) still genuinely unresolved vs live data — run \`npm run search:gaps:classify\`, then \`search:gaps:candidates\` / \`search:gaps:apply\` on refrigerator_water.`,
    );
  }
  if (stagedReviewingCount > 0) {
    recommended_next_actions.push(
      `${stagedReviewingCount} staged row(s) in "reviewing" — finish review or reject; filters missing brand -> \`npm run buckparts:staged-filter:review-brand\`; compat -> \`npm run search:gaps:review-compat:fridge\`.`,
    );
  }
  if (distinctCompatNoLink > 0) {
    recommended_next_actions.push(
      `${distinctCompatNoLink} filter(s) appear in compatibility_mappings but have zero retailer_links — add affiliate rows so model pages monetize (\`npm run buckparts:guardrails:refrigerator\` lists them).`,
    );
  }
  if (orphanFiltersCount > 0) {
    recommended_next_actions.push(
      `${orphanFiltersCount} live filter(s) are discovery orphans (no compat, no links) — map to models or add links; guardrail report: \`npm run buckparts:guardrails:refrigerator\`.`,
    );
  }
  if (recommended_next_actions.length === 0) {
    recommended_next_actions.push(
      "No items flagged in this snapshot; re-run after pipeline work. Schedule `npm run buckparts:guardrails:refrigerator` periodically.",
    );
  }
  ...
  const payload = {
    ...
    summary_counts: {
      ...
      live_fridge_models: totalFridgeModelsCount.count ?? 0,
      live_filters: totalFiltersCount.count ?? 0,
    ...
    interpretation: {
      ...
      new_live_in_window:
        "Live fridge_models / filters with created_at in the last --since-days (skipped if DB has no created_at column).",
      orphan_filters:
        "filters with no row in compatibility_mappings AND no row in retailer_links (matches discovery hiding rule).",
      search_gap_classification:
        "Each shown gap is replayed through searchCatalog(skipTelemetry) + filter RPCs + model_number_norm/oem_part_number_norm checks. Order: resolved by live search -> hidden orphan filter -> superseded by live row -> still unresolved.",
    },
    ...
    new_live_in_window: {
      fridge_models: newLiveModelsWindow,
      fridge_models_query_error: newLiveModelsWindowError,
      filters: newLiveFiltersWindow,
      filters_query_error: newLiveFiltersWindowError,
    },
    ...
    command_hints: {
      runbook: "npm run buckparts:runbook:refrigerator",
      guardrails: "npm run buckparts:guardrails:refrigerator",
      promote: "npm run search:gaps:promote:fridge -- --write",
      review_filter_brand: "npm run buckparts:staged-filter:review-brand",
      classification_env:
        "Gap replay uses `NEXT_PUBLIC_SUPABASE_*` anon client + `searchCatalog(..., { skipTelemetry: true })` (no search_events / search_gap writes). Service role still used for batched inventory reads.",
    },
  };
...
main().catch((e) => {
  console.error("[runbook-refrigerator] failed", e);
  process.exit(1);
});
