#!/usr/bin/env bash
# Phase 4 Decision-Capture v1 bounded validator (sibling to P4-ENTRY supply scoreboard).
...
terminal_fail() {
  ...
  echo "PHASE4_DECISION_CAPTURE_FAIL"
}
...
require_contract_doc() {
  [[ -f "$ROOT/docs/BuckParts-PHASE4-DECISION-CAPTURE-CONTRACT-V1.md" ]] ||
    fail_validator "missing docs/BuckParts-PHASE4-DECISION-CAPTURE-CONTRACT-V1.md"
  grep -q "phase4_decision_capture_v1" "$ROOT/docs/BuckParts-PHASE4-DECISION-CAPTURE-CONTRACT-V1.md" ||
    fail_validator "contract doc missing phase4_decision_capture_v1"
  grep -q "Raw inventory does not enter the denominator" \
    "$ROOT/docs/BuckParts-PHASE4-DECISION-CAPTURE-CONTRACT-V1.md" ||
    fail_validator "contract doc missing evidence-based denominator definition"
}

TMP_DIR="$(mktemp -d -t bp-p4-decision-capture-XXXXXX)"

case "${BUCKPARTS_PHASE4_DECISION_CAPTURE_INJECT:-}" in
  success_short)
    SUCCESS_COMPLETE=1
    echo "PHASE4_DECISION_CAPTURE_PASS"
    exit 0
    ;;
esac

require_contract_doc
run_group phase4_decision_capture_tests "$TESTS_TIMEOUT_SEC" \
  env BUCKPARTS_TEST_FILES='scripts/lib/buckparts-phase4-decision-capture-v1.test.ts' \
