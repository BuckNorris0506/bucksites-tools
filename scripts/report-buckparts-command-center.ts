...
} from "./lib/buckparts-phase4-outcome-capture-v1";
...
import { buildPhase1OperatingCircuitV1 } from "./lib/buckparts-phase1-operating-circuit-v1";
...
import { buildCommandCenterControlGraphRollupV1 } from "./lib/command-center-control-graph-rollup-v1";
...
import { buildCommandCenterIssueReauditLaneV1 } from "./lib/command-center-issue-reaudit-v1";
...
import { buildFridgeGuardedBatchLifecycleRulePromotionPlanCommandCenterLaneV1 } from "./lib/fridge-guarded-batch-lifecycle-rule-promotion-plan-command-center-v1";
...
export async function buildBuckpartsCommandCenterReport(
  ...
  const rpwfe_verified_link_rescue_plan_v1 = buildRpwfeVerifiedLinkRescuePlanV1({
    rootDir,
    fileExists,
    readTextFile,
    rpwfeOwnerReview: rpwfe_purchase_option_rescue_owner_review_v1,
  ...
  const rpwfe_official_ge_apply_plan_proposal_v1 = buildRpwfeOfficialGeApplyPlanProposalLaneV1({
    rootDir,
    fileExists,
    readTextFile,
  ...
  let manufacturer_safe_link_rescue_director_v1;
  try {
    manufacturer_safe_link_rescue_director_v1 =
      buildManufacturerSafeLinkRescueDirectorCommandCenterLaneV1({
        rootDir,
  ...
  let mission_factory_registry_v1;
  try {
    mission_factory_registry_v1 = buildMissionFactoryRegistryCommandCenterLaneV1({
      rootDir,
      now,
    });
  } catch (error) {
  ...
  const fridge_guarded_batch_lifecycle_rule_proposal_v1 =
    buildFridgeGuardedBatchLifecycleRuleProposalCommandCenterLaneV1(
      fridge_guarded_batch_closeout_learning_v1,
    );
  const fridge_guarded_batch_lifecycle_rule_promotion_plan_v1 =
    buildFridgeGuardedBatchLifecycleRulePromotionPlanCommandCenterLaneV1(
      fridge_guarded_batch_lifecycle_rule_proposal_v1,
    );

  const command_center_control_graph_rollup_v1 = buildCommandCenterControlGraphRollupV1({
    rootDir,
    now,
  });
  ...
  const universal_batch_lifecycle_apply_readiness_v1 =
    buildUniversalBatchLifecycleApplyReadinessCommandCenterLaneV1({
      rootDir,
      now,
      fileExists,
      readText: readTextFile,
  ...
  const universal_batch_lifecycle_mutation_authorization_review_v1 =
    buildUniversalBatchLifecycleMutationAuthorizationReviewCommandCenterLaneV1({
      rootDir,
      now,
      applyReadiness: universal_batch_lifecycle_apply_readiness_v1,
  ...
  const universal_batch_lifecycle_truth_table_v1 = buildUniversalBatchLifecycleTruthTableV1({
    ...
    mutation_authorization_review: universal_batch_lifecycle_mutation_authorization_review_v1,
    buckpartsScriptNames: buckpartsScriptNamesForLifecycle,
  });
  ...
  const command_center_issue_reaudit_v1 = buildCommandCenterIssueReauditLaneV1({
    rootDir,
    issue_registry: command_center_issue_registry_v1,
    live_site_monitor: liveSiteMonitor,
    now,
  });

  const seo_opportunity_registry_v1 = buildSeoOpportunityRegistryLaneV1({
    rootDir,
    now,
    fileExists,
    readTextFile,
  });
  const revenue_opportunity_registry_v1 = buildRevenueOpportunityRegistryLaneV1({
    rootDir,
    now,
    fileExists,
    readTextFile,
  ...
  const issueReauditSteeringOverride = resolveCommandCenterIssueReauditSteeringOverrideV1(
    command_center_issue_reaudit_v1,
  );
  ...
  const universalBatchLifecycleSteeringOverride = resolveUniversalBatchLifecycleSteeringOverrideV1({
    ...
    applyReadiness: universal_batch_lifecycle_apply_readiness_v1,
    applyExecutionPlan: universal_batch_lifecycle_apply_execution_plan_v1,
    mutationAuthorizationReview: universal_batch_lifecycle_mutation_authorization_review_v1,
  });
  ...
  let phase4_decision_capture_v1;
  try {
    phase4_decision_capture_v1 = buildPhase4DecisionCaptureV1({
      rootDir,
      now,
      demandNextLane: demand_to_coverage_next_lane_v1,
    });
  } catch (error) {
    phase4_decision_capture_v1 = buildPhase4DecisionCaptureUnknownV1({
      reason: error instanceof Error ? error.message : String(error),
      now,
    });
  }
  ...
  let phase4_outcome_capture_v1;
  try {
    phase4_outcome_capture_v1 = buildPhase4OutcomeCaptureV1({
      now,
      decisionCapture: phase4_decision_capture_v1,
      searchAndClick: searchAndClickSummary,
      clickVisibility:
