...
export type RepairClinicAirPurifierBlockedDetailsReport = {
  report_name: "buckparts_repairclinic_air_purifier_blocked_details_v1";
  generated_at: string;
  read_only: true;
  data_mutation: false;
...
export async function buildRepairClinicAirPurifierBlockedDetailsReport(
  options: BuildOptions = {},
): Promise<RepairClinicAirPurifierBlockedDetailsReport> {
  ...
  try {
    ...
    return {
      ...
      recommended_next_action:
        "Manually verify RepairClinic search rows for direct PDP evidence (token match + buyability) before any DB mutation.",
      known_unknowns: [],
    };
  } catch {
    return {
      ...
      sample_rows: "UNKNOWN",
      recommended_next_action:
        "RepairClinic blocked detail query unavailable; restore data access and rerun before evidence collection.",
      known_unknowns: ["air_purifier_retailer_links dataset unavailable for RepairClinic blocked detail extraction."],
    };
  }
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === THIS_FILE) {
  main().catch((error) => {
    console.error("[report-repairclinic-air-purifier-blocked-details] failed", error);
    process.exit(1);
  });
}
