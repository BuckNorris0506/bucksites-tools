/**
 * Manual search_gaps status updates for air-purifier-related rows only.
 * Dry-run by default; use --write to persist.
 */
import { loadEnv } from "./lib/load-env";
import {
  classifyAirPurifierSearchGap,
  type AirPurifierGapClassification,
...
const PAGE = 2000;
const WEDGE = HOMEKEEP_WEDGE_CATALOG.air_purifier;
...
function suggestedActionFromClassification(
  c: AirPurifierGapClassification,
): GapAction | null {
  if (
...
async function main() {
  ...
  const [fromCompat, fromLinks] = await Promise.all([
    pagedColumnIds("air_purifier_compatibility_mappings", "air_purifier_filter_id"),
    pagedColumnIds("air_purifier_retailer_links", "air_purifier_filter_id"),
  ]);
  const usefulFilterIds = new Set([...Array.from(fromCompat), ...Array.from(fromLinks)]);
  ...
  for (const gapId of ids) {
    ...
    if (!wedgeAllowsSearchGapCatalog(WEDGE, g.catalog)) {
      rows.push({
        gap_id: gapId,
        error: `catalog "${g.catalog}" is not air-purifier-scoped (allowed: ${ALLOWED_CATALOG_LIST.join(", ")})`,
        row: g,
      });
      continue;
    }
    ...
    JSON.stringify(
      {
        ...
        read_only: !write,
        scope: "air_purifier_related_search_gaps",
        allowed_catalogs: [...ALLOWED_CATALOG_LIST],
        action: targetStatus,
        hint:
          "Runbook: `npm run buckparts:runbook:air-purifier` for classification across top gaps. This script does not auto-pick ids.",
        rows,
      },
      null,
      2,
  ...
  if (!write) {
    console.error(
      "[apply-search-gap-status-air-purifier] dry-run only. Re-run with --write to update search_gaps.status.",
    );
  }
}

main().catch((e) => {
  console.error("[apply-search-gap-status-air-purifier] failed", e);
  process.exit(1);
});
