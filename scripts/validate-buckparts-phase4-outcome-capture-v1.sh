#!/usr/bin/env bash
# Phase 4 Outcome-Capture v1 bounded validator (final Phase 4 sibling).
...
terminal_fail() {
  ...
  echo "PHASE4_OUTCOME_CAPTURE_FAIL"
}
...
require_contract_doc() {
  [[ -f "$ROOT/docs/BuckParts-PHASE4-OUTCOME-CAPTURE-CONTRACT-V1.md" ]] ||
    fail_validator "missing docs/BuckParts-PHASE4-OUTCOME-CAPTURE-CONTRACT-V1.md"
  grep -q "phase4_outcome_capture_v1" "$ROOT/docs/BuckParts-PHASE4-OUTCOME-CAPTURE-CONTRACT-V1.md" ||
    fail_validator "contract doc missing phase4_outcome_capture_v1"
  grep -q "Never reward raw clicks" \
    "$ROOT/docs/BuckParts-PHASE4-OUTCOME-CAPTURE-CONTRACT-V1.md" ||
    fail_validator "contract doc missing Never reward raw clicks rule"
  grep -q "Goodhart" \
    "$ROOT/docs/BuckParts-PHASE4-OUTCOME-CAPTURE-CONTRACT-V1.md" ||
    fail_validator "contract doc missing Goodhart guard"
}

TMP_DIR="$(mktemp -d -t bp-p4-outcome-capture-XXXXXX)"

case "${BUCKPARTS_PHASE4_OUTCOME_CAPTURE_INJECT:-}" in
  success_short)
    SUCCESS_COMPLETE=1
    echo "PHASE4_OUTCOME_CAPTURE_PASS"
    exit 0
    ;;
esac

require_contract_doc
run_group phase4_outcome_capture_tests "$TESTS_TIMEOUT_SEC" \
  env BUCKPARTS_TEST_FILES='scripts/lib/buckparts-phase4-outcome-capture-v1.test.ts' \
