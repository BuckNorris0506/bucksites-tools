...
export function buildCodexNextExecutionPacketWrapperPrefixV1(): string {
  return [
    ...
    "- Do **not** run writable validation commands inside this read-only sandbox: **`npm run lint`**, **`npm run build`**, and **`npm run buckparts:operator-proof`** require writable `.next/*`, npm/tsx temp IPC, and similar paths — run those **only** on repo-owned Runner / local CI / a normal writable terminal after Codex inspection.",
    "- In your final message, list which external validation commands the operator should run outside Codex (see packet **EXTERNAL REPO VALIDATION BUNDLE**).",
    "- Return structured findings only: what you checked, pass/fail at read-only scope, what remains owner-only.",
    "",
    "---",
    "",
  ].join("\n");
