...
import { buildRepairClinicRefrigeratorBlockedDetailsReport } from "./report-repairclinic-refrigerator-blocked-details";
...
test("includes only repairclinic blocked search OEM rows", async () => {
  const report = await buildRepairClinicRefrigeratorBlockedDetailsReport({
    ...
    fetchRows: async () => [
      ...
      },
      {
        id: "drop-domain",
        retailer_key: "oem-catalog",
        affiliate_url: "https://www.example.com/Search?SearchTerm=DA29-00020B",
        browser_truth_classification: null,
      },
      {
        id: "drop-key",
  ...
  if (Array.isArray(report.sample_rows)) {
    ...
    assert.equal(report.sample_rows[0]?.detected_token, "DA29-00020B");
