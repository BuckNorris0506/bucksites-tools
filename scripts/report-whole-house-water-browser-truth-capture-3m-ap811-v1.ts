...
function main(): void {
  ...
  const result = buildWhw3mAp811BrowserTruthCaptureV1({ rootDir });

  let artifactRel: string | null = null;
  if (args.write) {
    artifactRel = writeWhwBrowserTruthCaptureResultV1({
      rootDir,
      result,
  ...
  process.stdout.write(
    `${JSON.stringify(
      {
        artifact_rel: artifactRel ?? WHW_AP811_BROWSER_TRUTH_RESULT_REL_V1,
        ...
        source_buyer_path_artifact: result.source_buyer_path_artifact,
        ...
        do_not_open_public: result.do_not_open_public,
