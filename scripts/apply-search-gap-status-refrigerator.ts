/**
 * Manual search_gaps status updates for refrigerator-related rows only.
...
function suggestedActionFromClassification(
  c: RefrigeratorGapClassification,
): GapAction | null {
  if (
...
async function main() {
  ...
  const [fromCompat, fromLinks] = await Promise.all([
    pagedColumnIds("compatibility_mappings", "filter_id"),
    pagedColumnIds("retailer_links", "filter_id"),
  ]);
  const usefulFilterIds = new Set([...fromCompat, ...fromLinks]);
  ...
  for (const gapId of ids) {
    ...
    if (!wedgeAllowsSearchGapCatalog(WEDGE, g.catalog)) {
      rows.push({
        gap_id: gapId,
        error: `catalog "${g.catalog}" is not refrigerator-scoped (allowed: ${ALLOWED_CATALOG_LIST.join(", ")})`,
        row: g,
      });
      continue;
    }
    ...
    JSON.stringify(
      {
        ...
        read_only: !write,
        scope: "refrigerator_related_search_gaps",
        allowed_catalogs: [...ALLOWED_CATALOG_LIST],
        action: targetStatus,
        hint:
          "Runbook: `npm run buckparts:runbook:refrigerator` for classification across top gaps. This script does not auto-pick ids.",
        rows,
      },
      null,
      2,
  ...
  if (!write) {
    console.error(
      "[apply-search-gap-status-refrigerator] dry-run only. Re-run with --write to update search_gaps.status.",
    );
  }
}

main().catch((e) => {
  console.error("[apply-search-gap-status-refrigerator] failed", e);
  process.exit(1);
});
