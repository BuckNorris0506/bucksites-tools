...
| C3 | **All mapped filters have proven safe buyer paths** | `buyer_path_truth_status === "PROVEN_SAFE_ROWS_EXIST"` (no `MIXED` or `ZERO_SAFE` on compat-mapped filters) **and** `safe_buyer_path_proven_count >= 1` | `MIXED` (partial proven — some mapped filters still zero-safe), `ZERO_SAFE_ROWS`, proven count `0`, or `UNKNOWN` (non-committed CSV / readiness unavailable) |
