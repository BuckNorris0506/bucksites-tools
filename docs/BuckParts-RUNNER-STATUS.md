...
**Last verified against repo:** 2026-05-15 (Founder Execution Packet — Codex sandbox vs Runner validation wording).
...
| Founder digest (markdown stdout) | `npm run buckparts:founder-digest` → `scripts/buckparts-founder-digest.ts`, `scripts/lib/buckparts-founder-digest-v1.ts` — **PROVEN:** optional `FOUNDER_DIGEST_RUNNER_STEP_JSON_PATH` + optional `FOUNDER_DIGEST_CODEX_PACKET_PROOF_JSON_PATH` (saved JSON from `buckparts:codex-next-execution-packet` stdout); digest reads files only — **PROVEN:** when proof JSON is PASS and `final_message_path` on that JSON points at a readable file (absolute or cwd-relative), digest builds `codex_output_review_packet_v1` markdown — **never** invokes Codex or Runner Step subprocesses from markdown generation. |
| Founder Action Queue | `src/lib/owner-dashboard/founder-action-queue-v1.ts` |
| Founder Decision Packets (owner-only v1) | `src/lib/owner-dashboard/founder-decision-packet-v1.ts` — **PROVEN:** read-only builder from Founder Action Queue; digest + dashboard; **not** agent execution prompts. **PROVEN:** Row-class shaping uses stable queue row ids from `founder-action-queue-v1.ts` (e.g. human browser, affiliate readiness, mutating gate, `next_best_action`, generic fallback). **PROVEN:** Founder Decision Registry v1 (`docs/BuckParts-FOUNDER-DECISION-REGISTRY.md`, `src/lib/owner-dashboard/founder-decision-registry-v1.ts`) defines how owner outcomes may be recorded. **INFERRED:** When Runner Step `overall_status` is `NO_PACKET`, owner decision `why_jared` appends that signal in digest CI runs. |
| Founder Decision Registry read model v1 | `npm run buckparts:founder-decision-registry` → `scripts/report-founder-decision-registry.ts` · `src/lib/owner-dashboard/founder-decision-registry-read-model-v1.ts` · `src/lib/owner-dashboard/founder-decision-registry-scan-v1.ts` — **PROVEN:** read-only JSON from `data/owner-decisions/*.json` when invoked via `node --import tsx scripts/report-founder-decision-registry.ts` (machine-parseable stdout; see `docs/BuckParts-JSON-STDOUT-CONTRACT.md`). **PROVEN:** optional `codex_output_review_context_v1` on rows is counted and digest-correlated to Codex Output Review `source_queue_row_id` (visibility only). **PROVEN:** weekly digest + owner dashboard embed read-model summary. **PROVEN:** counts and correlation do **not** alter Founder Action Queue, Decision Packets, Execution Packets, Runner Step, or mutation gates. |
| Failure Pattern Registry v1 (seeded read model) | `docs/BuckParts-FAILURE-PATTERN-REGISTRY.md` · `src/lib/owner-dashboard/failure-pattern-registry-v1.ts` — **PROVEN:** seeded failure classes with `guardrail_paths` cite in-repo docs/tests/workflows; digest + owner dashboard embed informational counts and row ids only. **PROVEN:** read model does **not** alter Runner Step allowlist, workflows, queues, packets, or mutation gates. |
| Layer 6 Readiness Summary v1 | `src/lib/owner-dashboard/layer-six-readiness-summary-v1.ts` — **PROVEN:** derives `readiness_status` from Failure Pattern Registry counts (blocked / needs_review / informational_ready); digest + owner dashboard surface summary only; optional Codex packet proof read model (`FOUNDER_DIGEST_CODEX_PACKET_PROOF_JSON_PATH`) may enrich external-agent evidence lines only; optional digest-built `codex_output_review_packet_v1` sets `codex_output_review_surface_v1` to PROVEN_PRESENT when `review_status` is READY_FOR_FOUNDER_REVIEW — **`codex_task_outcome_status`** on that packet separates captured prose from proven validation success (**TASK_SUCCESS_PROVEN** vs partial/failed/unknown); **PROVEN:** when digest also supplies Founder Decision Registry read model with digest queue correlation, `founder_decision_recording_for_codex_review_v1` may be **PROVEN_PRESENT** if a validated `codex_output_review_context_v1` row matches the same `source_queue_row_id` (read visibility only); **NOT PROVEN:** Layer 6 founder judgment loop complete, registry consumed by automation gates, or mutation authority from `approve_readonly_findings`; summary does **not** expand Runner autonomy. |
| JSON stdout contract (npm vs `tsx`) | `docs/BuckParts-JSON-STDOUT-CONTRACT.md` · `scripts/json-stdout-contract.test.ts` — **PROVEN:** `JSON.parse` / `jq` on stdout must not follow `npm run` for JSON scripts; use `node --import tsx scripts/…`. **INFERRED:** npm lifecycle banner is the recurring failure class. |
| HQ handoff freshness (Layer 6 section presence) | `docs/BuckParts-HQ-HANDOFF.md` §0B · `scripts/buckparts-hq-handoff-freshness.test.ts` — **PROVEN:** targeted test fails when required control-plane terms drop from HQ handoff; **PROVEN:** not wired into default `npm test` unless added there intentionally. |
| Batch Production Lane v1 (read-only report) | `docs/BuckParts-BATCH-PRODUCTION-LANE-V1.md` · `src/lib/owner-dashboard/batch-production-lane-v1.ts` · `node --import tsx scripts/report-batch-production-review.ts` — **PROVEN:** pure builder + stdout JSON (cap 10, `may_mutate: false` on every row); **NOT IMPLEMENTED:** digest/queue wiring or apply lane; Layer 6 **NOT_PROVEN** unchanged. |
...
| Codex + next Founder Execution Packet (JSON stdout summary) | `npm run buckparts:codex-next-execution-packet` → `scripts/run-buckparts-codex-next-execution-packet.ts` — **PROVEN:** builds snapshot via `buildNextExecutionPacketSnapshotV1` (same path as `buckparts:next-execution-packet`); if no eligible packet, stdout `overall_status: "NO_PACKET"` and exit 0; else prefixes repo `copy_paste_prompt` + read-only wrapper (no commits / no Supabase / no retailer_links / no evidence JSON / no affiliate edits; **PROVEN:** wrapper + packet forbid writable `lint`/`build`/`operator-proof` inside read-only sandbox) and runs `codex exec --sandbox read-only --json`; captures JSONL + final message under OS temp; fails on dirty `git status --short`. **NOT PROVEN:** founder-only approval (`layer_6_founder_only_approval: "NOT_PROVEN"`), mutation safety beyond sandbox + prompt, closed-loop Runner. **NOT wired** into Runner Step or workflows. |
| Codex Packet Proof read model v1 (digest/dashboard informational) | `src/lib/owner-dashboard/codex-packet-proof-read-model-v1.ts` — **PROVEN:** validates `buckparts_codex_next_execution_packet_v1` JSON; `codex_packet_proof_read_model_v1` with `automation_input: false`. **PROVEN:** digest optional env `FOUNDER_DIGEST_CODEX_PACKET_PROOF_JSON_PATH`; invalid JSON yields informational INVALID markdown without throwing. **PROVEN:** owner dashboard UNKNOWN modeled panel — no Codex/filesystem proof wiring in HTTP handler yet. **NOT PROVEN:** Layer 6 founder-only approval or mutation safety. |
| Codex Output Review Packet v1 (digest-only judgment surface) | `src/lib/owner-dashboard/codex-output-review-packet-v1.ts` — **PROVEN:** pure `codex_output_review_packet_v1` builder over proof read model + optional final-message text (digest loads file from PASS proof `final_message_path` only — **never** runs Codex). **PROVEN:** digest section `## Codex Output Review Packet (owner-only v1)` when proof env set; **`codex_task_outcome_status`** heuristically classifies final-message prose (`TASK_SUCCESS_PROVEN` / `TASK_PARTIAL_OR_FAILED` / `TASK_OUTCOME_UNKNOWN`) — failures win over success markers; common read-only sandbox signals include EPERM temp IPC, `tsx` temp IPC, `.next/cache`, `.next/trace`, `xcrun` temp paths. **PROVEN:** owner dashboard UNKNOWN modeled panel — HTTP handler does not read temp Codex paths. **NOT PROVEN:** Layer 6 complete, registry consumption, Runner automation inputs. |
...
| Founder Execution Packet v1 (read-only prompts) | `src/lib/owner-dashboard/founder-execution-packet-v1.ts` — **PROVEN:** `copy_paste_prompt` for agent-safe rows directs **read-only inspection / reporting only** inside Codex read-only sandbox; **PROVEN:** explicitly forbids running `npm run lint`, `npm run build`, and `npm run buckparts:operator-proof` inside that sandbox (writable `.next/*`, temp IPC). **PROVEN:** same three commands remain **`validation_command` + external acceptance criteria** for **Runner Step** (`npm run buckparts:runner-step`), local CI, or founder terminal — not Codex sandbox validation. |

**Packet eligibility (PROVEN):** `founder-execution-packet-v1.ts` emits packets only when `status === "agent_safe"` AND `recommended_actor === "agent"` AND `mutation_authority === "read_only"`. **`validation_command`** still lists `npm run lint`, `npm run build`, `npm run buckparts:operator-proof` as **external** acceptance criteria (Runner/local CI), not as in-sandbox Codex instructions.
...
**Codex digest proof split (PROVEN in repo):** `buckparts_codex_next_execution_packet_v1` PASS JSON evidences Codex **transport** + JSONL/final-message **capture** + clean git when flags hold; digest-built `codex_output_review_packet_v1` adds **`codex_task_outcome_status`** from final-message phrase heuristics — **capture ≠ proven lint/build/operator-proof success.**
...
| **Codex** | **PROVEN:** `npm run buckparts:codex-readonly-smoke` → `scripts/run-buckparts-codex-readonly-smoke.ts`; **PROVEN:** `npm run buckparts:codex-next-execution-packet` → `scripts/run-buckparts-codex-next-execution-packet.ts` (repo-built `copy_paste_prompt` + read-only wrapper); **PROVEN:** optional digest embed when `FOUNDER_DIGEST_CODEX_PACKET_PROOF_JSON_PATH` points at saved stdout JSON (`codex_packet_proof_read_model_v1`); **PROVEN:** digest may additionally embed `codex_output_review_packet_v1` when PASS proof JSON includes readable `final_message_path`. Both Codex CLIs require Codex CLI + successful `codex login status` on host. HQ handoff mentions credits separately. **NOT wired** into Runner Step/workflows. | **PARTIAL — bounded UNKNOWN:** proves **read-only** Codex subprocess + JSONL/final-message capture when invoked locally only; **`buckparts_codex_next_execution_packet_v1` PASS proves transport/capture, not necessarily lint/build/operator-proof success** — latest observed live packet prose showed EPERM / `.next/*` write blocks / `xcrun` temp issues under read-only sandbox while capture still succeeded (**INFERRED:** classify via digest `codex_task_outcome_status`). Digest proof/review sections do **not** prove founder-only approval, mutation safety beyond sandbox/prompt, closed-loop autonomy, or Cursor/OpenAI parity. |
...
| Date | Change |
|------|--------|
| 2026-05-08 | **Layer 6 Readiness Summary v1:** `layer_six_readiness_summary_v1` — **PROVEN:** readiness is summarized from Failure Pattern Registry guardrail counts; **PROVEN:** remains informational and does not expand autonomy (Layer 6 still NOT_PROVEN). |
| 2026-05-08 | **Failure Pattern Registry v1:** `failure_pattern_registry_read_model_v1` + `docs/BuckParts-FAILURE-PATTERN-REGISTRY.md` + `src/lib/owner-dashboard/failure-pattern-registry-v1.ts` — **PROVEN:** records recurring process/build failure classes mapped to guardrail paths; **PROVEN:** digest + dashboard surface informational counts only; **PROVEN:** does not expand autonomy (no Runner/queue/packet/gate inputs). |
...
| 2026-05-15 | **JSON stdout contract:** `docs/BuckParts-JSON-STDOUT-CONTRACT.md` + `scripts/json-stdout-contract.test.ts` — **PROVEN:** machine `JSON.parse` / `jq` must not consume `npm run` stdout for JSON BuckParts scripts; use `node --import tsx scripts/…`. |
| 2026-05-16 | **Founder Decision Registry ↔ Codex Output Review (read visibility):** optional `codex_output_review_context_v1` on registry rows; read-model counts + digest queue correlation; Layer 6 summary field `founder_decision_recording_for_codex_review_v1` — **NOT PROVEN:** automation gate consumption or Layer 6 complete. |
| 2026-05-15 | **Founder Decision Registry read model v1:** `founder_decision_registry_read_model_v1` + `npm run buckparts:founder-decision-registry` — **PROVEN:** repo can read/validate `data/owner-decisions/*.json` and surface counts in digest/dashboard; **PROVEN:** read path still does not change automation (queues, packets, Runner). |
| 2026-05-15 | **Founder Decision Registry v1:** `docs/BuckParts-FOUNDER-DECISION-REGISTRY.md` + `src/lib/owner-dashboard/founder-decision-registry-v1.ts` + optional `data/owner-decisions/` — contract for recording owner decisions; **PROVEN:** optional JSON is read-only in digest/report paths (no writes); **PROVEN:** registry rows are not automation inputs for queues, packets, or Runner. |
...
| 2026-05-15 | **Codex task outcome heuristic (`codex_task_outcome_status`):** transport/capture proof (`buckparts_codex_next_execution_packet_v1` PASS) split from final-message validation outcome classification in `codex_output_review_packet_v1`; Layer 6 readiness warns when surface is PROVEN_PRESENT but outcome is not TASK_SUCCESS_PROVEN; **INFERRED:** latest live packet prose cited EPERM / `.next/*` / `xcrun` sandbox friction while capture still succeeded. |
| 2026-05-15 | **Codex Output Review Packet v1:** `codex_output_review_packet_v1` (`src/lib/owner-dashboard/codex-output-review-packet-v1.ts`) — **PROVEN:** digest builds owner judgment markdown from proof JSON + readable `final_message_path` when env proof path set; Layer 6 readiness exposes `codex_output_review_surface_v1`; owner dashboard stays UNKNOWN for review until HTTP artifact wiring exists; **NOT PROVEN:** mutation authority, Runner inputs, Layer 6 completion. |
| 2026-05-15 | **Founder Execution Packet v1 + Codex wrapper:** `founder-execution-packet-v1.ts` + `run-buckparts-codex-next-execution-packet.ts` — **PROVEN:** read-only packet copy forbids `npm run lint` / `npm run build` / `npm run buckparts:operator-proof` inside Codex read-only sandbox; **PROVEN:** same commands remain external acceptance criteria (Runner Step / local CI); **PROVEN:** `codex_output_review_packet_v1` classifier treats `tsx` temp IPC + refined EPERM / `.next/*` failure lines as sandbox-aware signals while keeping `TASK_PARTIAL_OR_FAILED` when those validations were attempted and failed. |
...
| 2026-05-16 | **`codex_packet_proof_read_model_v1`** (`src/lib/owner-dashboard/codex-packet-proof-read-model-v1.ts`) + optional `FOUNDER_DIGEST_CODEX_PACKET_PROOF_JSON_PATH` on founder digest — **PROVEN:** informational validation of saved `buckparts_codex_next_execution_packet_v1` JSON; Layer 6 digest hint + readiness summary mention; owner dashboard UNKNOWN panel (no artifact read in HTTP handler); **NOT PROVEN:** founder approval loop. |
| 2026-05-16 | **HQ handoff freshness guard:** `docs/BuckParts-HQ-HANDOFF.md` §0B + `scripts/buckparts-hq-handoff-freshness.test.ts` — **PROVEN:** audit-oriented presence check for Layer 6 / Codex / Runner / registry terms; run via `node --import tsx --test scripts/buckparts-hq-handoff-freshness.test.ts`. |
| 2026-05-16 | **Batch Production Lane v1 contract:** `docs/BuckParts-BATCH-PRODUCTION-LANE-V1.md` — review-only candidate batches (5–10); implementation **NOT_PROVEN**. |
| 2026-05-17 | **Batch Production Lane v1 report:** `batch-production-lane-v1.ts` + `report-batch-production-review.ts` — read-only stdout JSON; hard cap 10; no apply script. |
