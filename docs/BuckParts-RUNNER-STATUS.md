...
**Last verified against repo:** 2026-05-16 (Codex packet proof read model + digest env wiring).
...
| Founder digest (markdown stdout) | `npm run buckparts:founder-digest` → `scripts/buckparts-founder-digest.ts`, `scripts/lib/buckparts-founder-digest-v1.ts` — **PROVEN:** optional `FOUNDER_DIGEST_RUNNER_STEP_JSON_PATH` + optional `FOUNDER_DIGEST_CODEX_PACKET_PROOF_JSON_PATH` (saved JSON from `buckparts:codex-next-execution-packet` stdout); digest reads files only — **never** invokes Codex or Runner Step subprocesses from markdown generation. |
