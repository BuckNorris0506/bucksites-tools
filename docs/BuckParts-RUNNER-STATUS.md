...
| Founder Decision Registry read model v1 | `npm run buckparts:founder-decision-registry` → `scripts/report-founder-decision-registry.ts` · `src/lib/owner-dashboard/founder-decision-registry-read-model-v1.ts` · `src/lib/owner-dashboard/founder-decision-registry-scan-v1.ts` — **PROVEN:** read-only JSON stdout from `data/owner-decisions/*.json`; weekly digest + owner dashboard embed read-model summary. **PROVEN:** counts do **not** alter Founder Action Queue, Decision Packets, Execution Packets, Runner Step, or mutation gates. |
