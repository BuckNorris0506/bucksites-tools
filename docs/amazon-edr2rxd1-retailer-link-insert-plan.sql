...
-- Token: EDR2RXD1 | Queue resolver: slug_lower → filter slug edr2rxd1
-- filter_id: ccc6209e-dafb-43c0-a1e7-8469897257d3
-- Evidence (committed backlog): data/evidence/amazon-edr2rxd1-oem-pdp-evidence.2026-05-04.json
-- ASIN: B00VBP8QPO | product_attribution: oem_official
...
FROM public.filters
WHERE id = 'ccc6209e-dafb-43c0-a1e7-8469897257d3'::uuid
   OR lower(slug) = lower('edr2rxd1')
   OR upper(oem_part_number) = upper('EDR2RXD1')
ORDER BY id;
...
WHERE filter_id = 'ccc6209e-dafb-43c0-a1e7-8469897257d3'::uuid
...
FROM public.retailer_links
WHERE (affiliate_url ilike '%/dp/B00VBP8QPO%' OR destination_url ilike '%/dp/B00VBP8QPO%')
  AND filter_id <> 'ccc6209e-dafb-43c0-a1e7-8469897257d3'::uuid
...
--   browser_truth_checked_at
-- ) values (
--   'ccc6209e-dafb-43c0-a1e7-8469897257d3'::uuid,
--   'Amazon',
--   'amazon',
--   'https://www.amazon.com/dp/B00VBP8QPO?tag=buckparts20-20',
--   'https://www.amazon.com/dp/B00VBP8QPO',
--   false,
--   'amazon',
--   'approved',
--   'manual',
...
--   'OEM-style PDP evidence; ASIN B00VBP8QPO; token EDR2RXD1. Evidence file: data/evidence/amazon-edr2rxd1-oem-pdp-evidence.2026-05-04.json.',
--   now()
-- );
-- commit;
